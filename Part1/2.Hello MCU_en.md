...
- [Show me your Git](#Show_Me_Your_Git)
- [Hello Cube](#Hello_Cube)
- [Show me the Clock](#Show_Me_The_Clock)
- [Reference](#Reference)
- [History](#history)
...
- Top5 suppliers [NXP](https://www.nxp.com/), [Microchip](https://www.microchip.com/), [Renesas](https://www.renesas.com/), [ST](https://www.st.com/content/st_com/en.html), [Infineon](https://www.infineon.com/), [accounted for 82%](https://www.semiconportal.com/archive/editorial/market/220616-mcuranking.html), and design standardization is progressing considerably by [ARM](https://www.arm.com/).
- In this chapter we're gonna practice the de facto VCS [Git](https://git-scm.com/) and use the [evaluation board (NUCLEO-G0B1RE)](https://www.st.com/ja/evaluation-tools/nucleo-g0b1re.html) from [ST](https://www.st.com/en/), one of the five major supplier in the micro controller market, with step by step approach.
...
<div id="Show_Me_Your_Git"></div>
<details open>
<summary><font size="5"><b>Show me your Git</b></font></summary>
...
<div id="Show_Me_The_Clock"></div>
<details open>
<summary><font size="5"><b>Show me the Clock</b></font></summary>
...
- From the「6.7 OSC clock sources」P.23 on the [User manual (mb1360)](https://www.st.com/resource/en/user_manual/um2324-stm32-nucleo64-boards-mb1360-stmicroelectronics.pdf), there are LSE (Low Speed External oscillator, X2, 32.768KHz), MCO (Micro controller Clock Output, X1, 8Mhz), HSE (High Speed External oscillator, X3, Not supported). <br>
![Oscillator](https://github.com/gtuja/CSC_MS/blob/main/Resources/Part1/Part1_Oscillator_en.png)
...
- Based on these information, let's make the clock maximum, 64Mhz.
- Open the xxx.ioc file, click the Pin Out Configuration : System Core -> RCC (Reset and Clock Controller) and set the HSE "BYPASS Clock Source".<br>
![RCC](https://github.com/gtuja/CSC_MS/blob/main/Resources/Part1/Part1_RCC_Setting.png)
- Change the MUX(Multiplexer) to "HSE", set the PLLM(Phase-Locked Loop Multiplier) where the clock is 64Mhz.<br>
![PLLM](https://github.com/gtuja/CSC_MS/blob/main/Resources/Part1/Part1_PLLM_Setting.png)
- Save the xxx.ioc, Cube IDE shall generate codes automatically.
- Thought this would be fine, but it seemed to stop at Error Handler!<br>
...
- Looked into what's wrong, it seems that MCO is not available with default setting.[User manual (UM2324)](https://www.st.com/resource/en/user_manual/um2324-stm32-nucleo64-boards-mb1360-stmicroelectronics.pdf) P.27
- There are two approaches, short SB17 with soldering iron or use the internal clock.
- Here we're gonna practice the internal clock setting as well.
- Create an empty project and implement LED toggling process below.
...
```
- Change the clock setting(16MHz/4Mhz), and check the LED blinking<br>
...
- Since it is managed by Git, you can check what has been changed by looking at the differences.
